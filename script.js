document.addEventListener('DOMContentLoaded', () => {
    ...
    if (timelineGrid) {
        const timelineImageContainer = document.createElement('div');
        timelineImageContainer.className = 'relative h-[500px]';
        
        const timelineTextContainer = document.createElement('div');
        timelineTextContainer.className = 'relative flex flex-col justify-center';
        ...
        timelineEvents.forEach((event, index) => {
            ...
            timelineTextContainer.innerHTML += `
                <div class="timeline-text absolute inset-0 flex flex-col justify-center transition-all duration-500" style="opacity: ${index === 0 ? 1 : 0}; transform: translateY(0);">
        ...
        timelineGrid.appendChild(timelineImageContainer);
        timelineGrid.appendChild(timelineTextContainer);
    }
    ...
    if(aboutSection) {
        ...
        window.addEventListener('scroll', () => {
            ...
            if (top < window.innerHeight && top > -height) {
                ...
                const progress = Math.max(0, Math.min(1, -top / scrollableDist));
